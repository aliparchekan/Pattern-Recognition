...
# poly one vs one
start = time()
parameters = {'degree' : np.linspace(1,10,10)}
svr = svm.SVC(kernel='poly', decision_function_shape='ovo', gamma='auto')
...
start = time()
parameters = {'degree' : np.linspace(1,10,10)}
svr = svm.SVC(kernel='poly', decision_function_shape='ovr', gamma='auto')
clf = GridSearchCV(svr, parameters)
clf.fit(train_data, train_label)
print('train duration is {}'.format(time() - start))
start = time()
...
svr = svm.SVC(kernel='linear', decision_function_shape='ovo', gamma='auto')
clf = GridSearchCV(svr, parameters)
clf.fit(train_data, train_label)
print('train duration is {}'.format(time() - start))
start = time()
...
start = time()
parameters = {'C' : np.linspace(1,10,10)}
svr = svm.SVC(kernel='linear', decision_function_shape='ovr', gamma='auto')
clf = GridSearchCV(svr, parameters)
clf.fit(train_data, train_label)
print('train duration is {}'.format(time() - start))
start = time()
